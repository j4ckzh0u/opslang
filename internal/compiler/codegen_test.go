...
func TestGenerateSDKImportAliases(t *testing.T) {
	tests := []struct {
		...
		{"causal uses opscausal alias", `let x = causal.trace_pid(1)`, "opscausal", "opscausal.TracePID"},
		...
		{"package facts uses opspackagefacts alias", `let x = package_facts.collect([])`, "opspackagefacts", "opspackagefacts.Collect"},
