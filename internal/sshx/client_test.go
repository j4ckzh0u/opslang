...
func TestNewPool(t *testing.T) {
	t.Run("valid pool", func(t *testing.T) {
		...
		if pool.Available() != 0 {
			t.Errorf("expected initial available 0, got %d", pool.Available())
		}
	})
	...
	t.Run("invalid pool size", func(t *testing.T) {
		...
		_, err := NewPool(cfg, 0)
		if err == nil {
			t.Error("NewPool() expected error for size 0")
		}

		_, err = NewPool(cfg, -1)
		if err == nil {
			t.Error("NewPool() expected error for negative size")
		}
	})
...
func TestPool_Put(t *testing.T) {
	...
	// Create a client and put it back
...
func TestPool_Get_WithMockServer(t *testing.T) {
	server, err := newMockSSHServer("testpass")
	if err != nil {
		t.Fatalf("failed to create mock server: %v", err)
	}
	...
	// Get should return the same client
	...
	if client1 != client2 {
		t.Error("expected to get same client from pool")
	}
}
...
func TestPool_Exhausted(t *testing.T) {
	...
	_, err = pool.Get(ctx)
	if err == nil {
		t.Error("Get() should fail when pool is exhausted")
	}
}

func TestPool_Close_WithConnections(t *testing.T) {
	server, err := newMockSSHServer("testpass")
	if err != nil {
		t.Fatalf("failed to create mock server: %v", err)
	}
