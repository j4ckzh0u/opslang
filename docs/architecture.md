...
OpsLang 是一门面向运维领域的领域特定语言（DSL），旨在通过简洁的脚本完成复杂的运维操作。系统采用纯 Go 实现，支持无需语言运行时的远程执行、异构架构和结构化返回。

### 1.1 核心设计原则

- **纯 Go 生态**：所有依赖支持 `CGO_ENABLED=0` 交叉编译，无 cgo 依赖
- **结构化返回**：标准库函数一律返回结构体，禁止返回原始字符串
- **无脚本运行时依赖**：标准模块优先使用 Go 库或系统调用；SSH 传输层仍通过远端 exec 通道调用必要的系统工具，并对参数统一做 shell quoting
...
- **零语言运行时远程执行**：通过 SSH 下发预编译 Runner 或 AOT 二进制；目标机仍须具备所选模块需要的内核、权限和系统组件
...
- 每个函数返回 `(结构体, error)`
- 结构体带 JSON tag，支持序列化
- 标准操作不依赖 Python/Shell 脚本；少数系统集成模块会直接调用目标机已有工具，远程命令参数由 SSH 层安全转义
