...
**AOT 模式**：AST 编译为 Go 源码，`go build` 生成静态二进制。支持控制流、函数和 OpsLang 文件模块，适合复杂逻辑。
...
```bash
opsctl deploy script.ops --targets host1,host2,host3
```
...
```ops
let targets = [{"host": "web1", "user": "root", "dest": "/opt/app/releases/"}]
let options = {"checksum": true, "parallel": 10, "retries": 3}
file.distribute("/data/releases/app-v2.1.0.tar.gz", targets, options)
```

当前实现使用真实 SSH/SFTP，支持并行、重试和传输后 SHA-256 校验。1 万主机模拟测试覆盖调度、重试、校验和归档编排；压缩、断点续传和分层中继仍在 Roadmap。
...
- **SSH 客户端**：TOFU 主机密钥验证，连接池，SFTP 上传/下载
- **安全模块**：权限分级（read_only/admin/root）、审计日志、Ed25519 签名验证、资源限制

所有标准库函数返回结构体，不返回原始字符串。1157 个注册原子操作覆盖系统、文件、网络、进程、服务、包管理等 210 个类别。
...
- 第三方 Go 包导入（OpsLang 文件模块已经实现）
- 文件压缩、断点续传、分层中继和传输前内容去重
- 非 systemd 主机资源限制回退和部署自动回滚接入
